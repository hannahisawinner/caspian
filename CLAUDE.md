...
- Both pages send through the shared `form-submit.js` (`sendForm()`), which is the ONLY place the Apps Script `/exec` URL lives (`SHEET_URL`). Changing it breaks both forms. It also classifies failures and shows an inline `.form-error` message: a JSON `error` reply → "Something went wrong on our end"; no reply / unreadable HTML reply (a rare Google hiccup — the row may still have been saved) → "We couldn't confirm that your submission went through…". The button shows "Sending…" and is disabled while in flight, typed data is kept on error, and technical details go to the browser console only.
